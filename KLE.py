...
def eigen_value_solution(eta, L, var, Num_Root):
    ...
    #########################################################################
    def dff(x):
        dff = (2 * eta ** 2 * x - 1) * np.sin(x * L) + (eta ** 2 * x ** 2 - 1) * np.cos(x * L) * L - 2 * eta * np.cos(
            x * L) + 2 * eta * x * np.sin(x * L) * L
        return dff
...
#     for i in range(n):
#         f[i, :] = (eta * w[i] * torch.cos(w[i] * x) + torch.sin(w[i] * x)) / torch.sqrt(
#             (eta ** 2 * w[i] ** 2 + 1) * L / 2 + eta)
#
#     return f

def k2(kesi, f_x, f_y, mean_logk, lamda):
    logk = mean_logk + np.sum(np.sqrt(lamda) * f_x * f_y * kesi, axis=1)
    kk = np.exp(logk)
    return kk
...
def eigen_func2(n, w, eta, L, x):
    f = np.empty((n, len(x)))
    for i in range(n):
        f[i, :] = (eta * w[i] * np.cos(w[i] * x) + np.sin(w[i] * x)) / np.sqrt(
            (eta ** 2 * w[i] ** 2 + 1) * L / 2 + eta)
